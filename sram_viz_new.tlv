   ...
   /banks_init[m4_eval(M4_BANK_SIZE-1):0]
      \viz_alpha 
         initEach: function() {
               ...
               let r_addr_q_val = new fabric.Text("r_addr_q_val_" + this.getScope("banks_init").index.toString(), {
                  top: 300,
                  left: 100  + (this.getScope("banks_init").index) * 1050 + 14 * M4_MEM_SIZE,
                  fontSize: 14,
                  fontFamily: "monospace"
               })
               let sram_val = new fabric.Text("sram_addr_val_" + this.getScope("banks_init").index.toString(), {
                  top: 300,
                  left: 550 + (this.getScope("banks_init").index) * 1050 + 14 * M4_MEM_SIZE,
                  fontSize: 14,
                  fontFamily: "monospace"
               })
         ...
         \viz_alpha
            initEach: function() {
               let bank_val_box = new fabric.Rect({
                  top: 1.75 * M4_MEM_SIZE + this.getScope("vr_size").index * 50 ,
                  left: 140 * M4_BANK_SIZE + ((this.getIndex() % 4) * 1000) + (this.getIndex() / 4) * 120,
                  fill: "white",
                  width: 100,
                  height: 14,
                  stroke: "black"
               })
               let data = new fabric.Text(this.getIndex().toString() + ": ", {
                  top: 1.75 * M4_MEM_SIZE + this.getScope("vr_size").index * 50,
                  left: 141 * M4_BANK_SIZE + ((this.getIndex() % 4) * 1000) + (this.getIndex() / 4) * 120, // bank: 3 2 1 0 format
                  fontSize: 14,
                  fontFamily: "monospace"
               })
            ...
            renderEach: function() {
               ...
               /*let vrf = this.getScope("vr_size").index;
               let addr_i = this.svSigRef(`sram_tb.dut.addr_i[vrf]`).asInt() ;
               let we_i = this.svSigRef(`sram_tb.dut.we_i[vrf]`).asBool(false) ;
               let req_i = this.svSigRef(`sram_tb.dut.req_i[vrf]`).asBool(false) ;
               let wdata_i = this.svSigRef(`sram_tb.dut.wdata_i[vrf]`).asInt() ;
               
               var mod = (we_i && req_i) && (addr_i == (this.getScope("mem_size").index) + this.getScope("vr_size").index * 32); // selects which bank to write on*/
